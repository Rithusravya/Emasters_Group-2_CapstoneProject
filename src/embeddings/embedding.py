...
class CodeEmbedder:
    """Generates vector embeddings for code and queries. Forces CPU for stability."""
    ...
    def __init__(
        ...
        if isinstance(model_name, str):
            logger.info(f"Loading embedder model '{model_name}' on CPU...")
            ...
            self.model_name = getattr(
                model_name, "name_or_path", "BAAI/bge-small-en-v1.5"
            )
    ...
    def generate_embedding(
        ...
        is_query: bool = False,
        normalize: bool = True,
        batch_size: int = 32,  # Increased batch size for CPU efficiency
    ) -> np.ndarray:
        ...
        if isinstance(data, pd.DataFrame):
            col_match = next(
                (
                    c
                    for c in ["code", "SQL", "sql", "query", "question", "text"]
        ...
        for i in range(0, len(processed_texts), batch_size):
            ...
            with torch.no_grad():
                ...
                all_embeddings.append(
                    sentence_embeddings.to(torch.float32).cpu().numpy()
                )
